...
export const ComparisonPanel: React.FC = () => {
  ...
  return (
    <motion.div
      ...
      className="fixed right-6 top-24 w-96 bg-slate-950/85 backdrop-blur-2xl border border-white/10 p-6 rounded-3xl shadow-2xl z-40 overflow-hidden"
    >
      ...
      <div className="space-y-3 relative z-10 mb-5">
        {[ 
          ...
          <div key={algo.name} className={`bg-linear-to-r ${algo.color} p-4 rounded-2xl border backdrop-blur-md`}>
            <div className="flex justify-between items-center mb-2">
              <h3 className="text-white font-semibold text-sm">{algo.name}</h3>
              <span className="text-[11px] font-mono text-slate-300">{algo.data.pathLength} steps path</span>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs text-slate-300">
              <div className="flex items-center gap-1.5 bg-black/30 px-2.5 py-1.5 rounded-xl border border-white/5">
                <Clock size={13} className="text-slate-400" /> 
                <span className="font-mono">{algo.data.time}</span>
              </div>
              <div className="flex items-center gap-1.5 bg-black/30 px-2.5 py-1.5 rounded-xl border border-white/5">
                <Target size={13} className="text-slate-400" /> 
                <span className="font-mono">{algo.data.visitedCount} nodes</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="relative z-10 bg-white/3 border border-white/10 p-4 rounded-2xl mb-5">
      ...
      <div className="relative z-10 pt-3 border-t border-white/10 flex items-center justify-between">
        ...
        <span className="text-xs font-bold font-mono px-3 py-1 bg-emerald-500/10 border border-emerald-500/30 text-emerald-300 rounded-full tracking-wider">
          {winnerName}
        </span>
      </div>
    </motion.div>
