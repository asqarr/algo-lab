  ...
  ({ node, onMouseDown, onMouseEnter, onMouseUp }) => {
    ...
    return (
      <div
        ...
        onMouseUp={onMouseUp}
      >
        {isWeight && <Zap className="w-3 h-3 text-fuchsia-200 pointer-events-none" />}

        {isDefaultEmptyNode && (
          <span className="w-1 h-1 rounded-full bg-slate-800/80 pointer-events-none" />
        )}
      </div>
    );
  },
