...
export const AlgorithmSelectorModal: React.FC<AlgorithmSelectorModalProps> = ({
  ...
  const algorithms = [
    {
      ...
      icon: <Cpu className="w-5 h-5 text-blue-400 animate-pulse" />,
    ...
    },
    {
      ...
      icon: <GitCommit className="w-5 h-5 text-purple-400 animate-pulse" />,
    ...
    },
    {
      ...
      icon: <FastForward className="w-5 h-5 text-amber-400 animate-pulse" />,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-hidden">
      ...
      <div
        className={`relative w-full max-w-2xl bg-[#0b0f19] border border-slate-800/80 rounded-3xl shadow-2xl p-6 md:p-7 overflow-hidden z-10 transition-all duration-300 ease-out transform ${
          animateIn
      ...
      >
        ...
        <div className="relative flex items-center justify-between pb-5 mb-5 border-b border-slate-800/80">
          <div className="flex items-center gap-3">
            <div className="p-2.5 rounded-2xl bg-indigo-500/10 border border-indigo-500/20 shadow-inner">
              <Sparkles className="w-5 h-5 text-indigo-400 animate-pulse" />
            </div>
            <div>
              <h3 className="text-xl font-bold text-white tracking-wide">Select Pathfinding Algorithm</h3>
              <p className="text-xs text-slate-400 mt-0.5">Choose an algorithm below to visualize its search strategy</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="relative grid grid-cols-1 gap-3.5 max-h-[58vh] overflow-y-auto pr-1.5 custom-scrollbar">
          {algorithms.map((algo) => (
            <div
              key={algo.id}
              className="group relative flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 rounded-2xl bg-slate-900/70 border border-slate-800/80 hover:border-slate-700/80 hover:bg-slate-900/90 transition-all duration-300 hover:shadow-xl hover:shadow-black/40"
            >
              <div className="flex items-start gap-3.5">
                <div className="p-3 rounded-2xl bg-slate-800/80 border border-slate-700/50 shadow-inner mt-0.5 sm:mt-0 group-hover:scale-105 transition-transform duration-300">
                  {algo.icon}
                </div>
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <h4 className="font-bold text-white text-base tracking-wide">{algo.name}</h4>
                    <span className={`text-[11px] px-2.5 py-0.5 rounded-full border font-medium ${algo.badgeColor}`}>
                      {algo.badge}
                    </span>
                  </div>
                  <p className="text-xs text-slate-400 mt-1.5 leading-relaxed">{algo.description}</p>
                  <div className="flex items-center gap-2 mt-2 text-[11px] text-slate-300 font-medium bg-slate-950/40 px-2.5 py-1 rounded-lg w-fit border border-slate-800/50">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-sm shadow-emerald-400"></span>
                    <span>Feature: {algo.feature}</span>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                className={`w-full sm:w-auto flex items-center justify-center gap-2 px-5 py-2.5 text-white rounded-xl text-sm font-semibold transition-all duration-300 cursor-pointer shadow-lg shrink-0 border border-white/10 active:scale-95 ${algo.btnBg}`}
              >
